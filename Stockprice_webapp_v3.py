...
tickerInfo = tickerData.quarterly_financials.reset_index()
...
fig3.add_trace(
    go.Table(
        header=dict(values=list(tickerInfo.columns),
                    fill_color='paleturquoise',
                    align='left'),
        cells=dict(values=[tickerInfo.iloc[:,0], tickerInfo.iloc[:,1], tickerInfo.iloc[:,2], tickerInfo.iloc[:,3], tickerInfo.iloc[:,4]],
                   fill_color='lavender',
                   align='left'))
    )

fig3.update_layout(width=wide, height=1200, font=dict(size=14))
    
st.plotly_chart(fig)
st.plotly_chart(fig2)
st.header("Financial summary table for " + stock)
